...
const CHAR_COLORS = ["#4a9eff", "#cc9900", "#e05c00", "#c84040", "#28c898", "#9a6fff", "#ff8c42"];
...
function CharacterCard({ character, index }) {
  ...
  const initial = character.name.split(" ").map((p) => p[0]).join("").slice(0, 2);
    ...
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: "0.8rem" }}>
        <div style={{
          ...
          fontFamily: "'Rajdhani', sans-serif", fontWeight: 700, fontSize: "0.8rem", color,
        }}>
          {initial}
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", flexWrap: "wrap", marginBottom: "0.25rem" }}>
            <span style={{ fontFamily: "'Rajdhani', sans-serif", fontWeight: 600, fontSize: "0.95rem", color: "#d8e8f2", letterSpacing: "0.04em" }}>
              {character.name}
            </span>
            <span style={{ fontFamily: "'Rajdhani', sans-serif", fontSize: "0.52rem", color: color + "aa", letterSpacing: "0.1em", border: `1px solid ${color}33`, padding: "0.1rem 0.4rem", borderRadius: "2px" }}>
              {character.role?.toUpperCase()}
            </span>
            <span style={{ fontFamily: "'Rajdhani', sans-serif", fontSize: "0.5rem", letterSpacing: "0.1em", color: statusCfg.color, border: `1px solid ${statusCfg.color}44`, padding: "0.1rem 0.35rem", borderRadius: "2px" }}>
              {statusCfg.label}
            </span>
            {character.type === "npc" && (
              <span style={{ fontFamily: "'Rajdhani', sans-serif", fontSize: "0.48rem", letterSpacing: "0.1em", color: "#3d6078", border: "1px solid #1aadad22", padding: "0.1rem 0.35rem", borderRadius: "2px" }}>
                NPC
              </span>
            )}
          </div>
          {character.notes && (
            <p style={{ fontSize: "0.77rem", color: "#6a8a9a", lineHeight: 1.7, margin: 0 }}>
              {character.notes}
            </p>
          )}
        </div>
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "baseline", gap: "0.6rem", marginBottom: "0.25rem", flexWrap: "wrap" }}>
        ...
        {vessel.designation && (
          <span style={{ fontFamily: "'Rajdhani', sans-serif", fontSize: "0.55rem", color: "#2a80e8", letterSpacing: "0.1em", border: "1px solid #2a60c044", padding: "0.1rem 0.4rem", borderRadius: "2px" }}>
            {vessel.designation}
          </span>
        )}
        {vessel.owner && (
          <span style={{ fontSize: "0.65rem", color: "#3d6078" }}>{vessel.owner}</span>
        )}
      </div>
      {vessel.notes && (
        <p style={{ fontSize: "0.77rem", color: "#6a8a9a", lineHeight: 1.7, margin: 0 }}>
...
function EventEntry({ event, index }) {
  return (
    <div style={{
      ...
      <div style={{
        fontFamily: "'Rajdhani', sans-serif", fontSize: "0.6rem", color: "#1aadad44",
        flexShrink: 0, paddingTop: "2px", width: "1.5rem", textAlign: "right",
      }}>
        {index + 1}
      </div>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontFamily: "'Rajdhani', sans-serif", fontWeight: 600, fontSize: "0.8rem", color: "#9ab8c8", letterSpacing: "0.04em", marginBottom: "0.2rem" }}>
          {event.story_title}
        </div>
        <p style={{ fontSize: "0.75rem", color: "#5a7888", lineHeight: 1.65, margin: 0 }}>
          {event.summary}
        </p>
      </div>
    </div>
...
export default function Codex() {
  ...
  if (loading) return (
    <div style={{ minHeight: "100vh", background: "#04050a", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "monospace", color: "#1aadad", fontSize: "0.75rem", letterSpacing: "0.3em" }}>
      <style>{styles}</style>
      LOADING CODEX...
    </div>
  );
  ...
  if (error || !world) return (
    <div style={{ minHeight: "100vh", background: "#04050a", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "monospace", color: "#d04040", fontSize: "0.75rem" }}>
      <style>{styles}</style>
    ...
    <div style={{ minHeight: "100vh", background: "#04050a", fontFamily: "'Share Tech Mono', monospace" }}>
      ...
      <div style={{ background: "#080a16", borderBottom: "2px solid #1aadad22", position: "sticky", top: 0, zIndex: 10 }}>
        <div style={{
          ...
          <button
            onClick={() => navigate("/")}
            style={{ background: "#0c1222", color: "#1aadad", border: "1px solid #1aadad44", fontFamily: "'Rajdhani', sans-serif", fontWeight: 600, fontSize: "0.6rem", letterSpacing: "0.1em", padding: "0.35rem 0.8rem", borderRadius: "12px", textTransform: "uppercase" }}
            onMouseEnter={(e) => { e.target.style.background = "#1aadad22"; }}
            onMouseLeave={(e) => { e.target.style.background = "#0c1222"; }}
          >
            ← Campaigns
          </button>
        </div>
      ...
      <div style={{ maxWidth: "820px", margin: "0 auto", padding: "2.5rem 1.5rem", position: "relative", zIndex: 1 }}>
        ...
        {/* Campaign stats bar */}
        <div style={{
          display: "flex", gap: "2rem", padding: "1rem 1.4rem",
          border: "1px solid #1aadad18", borderLeft: "3px solid #1aadad44",
          background: "rgba(4,6,12,0.8)", marginBottom: "2.5rem",
        ...
        {crew.length > 0 && (
          <div style={{ marginBottom: "2.5rem" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "1px" }}>
              {crew.map((c, i) => <CharacterCard key={c.name} character={c} index={i} />)}
            </div>
          </div>
        )}
        ...
        {npcs.length > 0 && (
          <div style={{ marginBottom: "2.5rem" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "1px" }}>
              {npcs.map((c, i) => <CharacterCard key={c.name} character={c} index={i} />)}
            </div>
          </div>
        )}
        ...
        {vessels.length > 0 && (
          <div style={{ marginBottom: "2.5rem" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "1px" }}>
              {vessels.map((v, i) => <VesselCard key={v.name} vessel={v} index={i} />)}
            </div>
          </div>
        )}
        ...
        {events.length > 0 ? (
          <div style={{ marginBottom: "2.5rem" }}>
            ...
            <div>
              {events.map((e, i) => <EventEntry key={e.story_id || i} event={e} index={i} />)}
            </div>
          </div>
        ) : (
          <div style={{ border: "1px dashed #1aadad15", padding: "2rem", textAlign: "center" }}>
            <div style={{ fontFamily: "'Rajdhani', sans-serif", fontSize: "0.7rem", color: "#283848", letterSpacing: "0.2em" }}>
              NO MISSIONS ON RECORD YET
            </div>
            <div style={{ fontSize: "0.7rem", color: "#3d5060", marginTop: "0.5rem" }}>
